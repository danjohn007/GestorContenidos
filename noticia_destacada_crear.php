<?php
/**
 * Crear Noticia Destacada (Solo Imagen)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Si no hay imagen de noticia, verificar si se subió archivo
    if (!$imagen_url && isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    ...
    } elseif (!$imagen_url && isset($_FILES['imagen']) && $_FILES['imagen']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if (!$imagen_url) {
        $errors[] = 'Debe proporcionar una imagen o seleccionar una noticia con imagen';
    }
    ...
    if (empty($errors)) {
        ...
        $result = $noticiaDestacadaImagenModel->create($data);
        if ($result) {
            setFlash('success', 'Noticia destacada creada exitosamente');
            redirect('noticias_destacadas.php');
        } else {
            $errors[] = 'Error al crear la noticia destacada. Intente nuevamente.';
        }
    }
}
