    ...
    elseif (isset($_FILES['imagen_destacada']) && $_FILES['imagen_destacada']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $allowedExtensions)) {
            ...
            if (move_uploaded_file($_FILES['imagen_destacada']['tmp_name'], $uploadPath)) {
                // Only delete old image if it's from noticias folder (not from multimedia gallery)
                if ($noticia['imagen_destacada'] && strpos($noticia['imagen_destacada'], '/public/uploads/noticias/') === 0) {
